...
const Nav: FC = () => {
  ...
  return (
    ...
    >
      <motion.div
        className={
          isTabletOrMobile ? "flex justify-between items-center w-full w-full-mh" : ""
        }
        layout
      >
        <motion.div
          className="flex space-x-6 items-center profile"
          onHoverStart={() => {
        ...
        {isTabletOrMobile && (
          <div onClick={() => setHamburgerState(!hamburgerState)}>
            <motion.div
              ...
              className={`bg-white h-0.5 rounded-full w-9 ${(hamburgerState) ? 'w-full-mh':''}`}
             />
            <motion.div
              animate={{
                rotate: hamburgerState ? -40 : 0,
      ...
      {!isTabletOrMobile && (
        <div className="text-[#FFFFFFFF] flex items-center space-x-16 rounded-md link-container">
          ...
          {/* <Link className="rounded-md border-4 border-sky-700 bg-sky-700 navlink" to="/cv.pdf" activeClass="active" smooth duration={200}>CV</Link> */}
          <a target="_blank" className="rounded-md border-4 border-sky-700 bg-sky-700 navlink" href="/cv.pdf">CV</a>
        </div>
      )}
      <AnimatePresence>
